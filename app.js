...
const APP = (() => {
  ...
  const META_DARK  = '#0b1520';
  const META_LIGHT = '#f0f4f8';
  ...
  let selectedImg = null;   // base64 dataURL
  let markerStore = {};     // id → metadata
  let markerCount = 0;
  let userMarker  = null;
  let accCircle   = null;
  let sheetOpen   = false;
  ...
  let manualTheme  = null;  // 'dark' | 'light' | null = auto

  /* ─────────────────────────────────────────────
     THEME
  ───────────────────────────────────────────── */

  /** Returns 'light' between 06:00 and 18:00, 'dark' otherwise */
  function autoTheme() {
  ...
  function applyTheme(theme) {
    ...
    document.getElementById('meta-theme').content =
      theme === 'light' ? META_LIGHT : META_DARK;
  ...
  // Re-check auto-theme every minute (for when app stays open across dawn/dusk)
  setInterval(() => {
    if (!manualTheme) applyTheme(autoTheme());
  }, 60_000);
  ...
  /* now we can safely init theme (tileLayer exists) */
  initTheme();
  ...
  function onPosition(pos) {
    ...
    if (!userMarker) {
      const icon = L.divIcon({
        ...
        html: `
          <div style="
            ...
            animation:_up 1.8s ease-out infinite;
          "></div>
          <style>
            @keyframes _up {
      ...
      accCircle  = L.circle(ll, {
        radius: acc, color: '#00e5ff',
        fillColor: '#00e5ff', fillOpacity: .05, weight: 1,
      }).addTo(map);
  ...
  function onError(err) {
    ...
    setStatus(false, msg[err.code] || 'Erro de GPS');
  }
  ...
  /* ─────────────────────────────────────────────
     IMAGE UPLOAD  (câmera & galeria separados)
  ───────────────────────────────────────────── */
  ...
  // Botão câmera → abre input com capture="environment"
  document.getElementById('btn-camera').addEventListener('click', () => {
    inputCamera.click();
  });

  // Botão galeria → abre input sem capture (galeria nativa)
  document.getElementById('btn-gallery').addEventListener('click', () => {
    inputGallery.click();
  });

  // Compartilham o mesmo handler de leitura
  ...
  /* confirm button enabled only when BOTH category AND image are set */
  ...
  document.getElementById('confirm-btn').addEventListener('click', () => {
    ...
    if (userLat === null) {
      showToast('⚠ Aguarde a localização ser obtida');
      return;
    }
    ...
    // ── Icon (always has camera badge since photo is mandatory) ──
    const icon = L.divIcon({
      className: '',
      html: `
        <div class="urban-marker" style="background:${cat.color}22;border-color:${cat.color};">
    ...
    const popupHtml = `
      ...
      <div style="padding:12px 14px 0;font-family:'DM Sans',sans-serif;">
        <div style="
          ...
          font-size:.9rem;color:${cat.color};margin-bottom:4px;
      ...
      <button class="pop-photo-btn" style="color:${cat.color};"
              onclick="APP.openLightbox(${id})">
